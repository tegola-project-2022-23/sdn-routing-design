...
class Calculate:
    ...
    def build_request(self, id, auth_token, start, end, take_half):
        data = {
                "jsonrpc": "2.0",
                "method": "history.get",
                "params": {
        ...
        for item in ast.literal_eval(req.text)["result"]:
           ...
           #     continue
            ...
            # Sum values if there are multiple IDs, and convert into from bits to Mbs
            if datetime.datetime.fromtimestamp(int(clock)) in values.keys():
                values[datetime.datetime.fromtimestamp(int(clock))] += value / 1000000
            else:
                values[datetime.datetime.fromtimestamp(int(clock))] = value / 1000000
    ...
    def get_demands(self, start, end):
        token = self.get_auth_token()

        id_test = ["47123", "47124", "47119", "47112", "47851", "47866", "47855", "45791", "46539", "46682", "46683", 
           "46074", "44896", "44898", "48803", "48804", "48805", "48806", "52256", 
           "46404", "46407", "46410", "46411", "46412","45794", "45793", "45807"]


        return self.build_request(id_test, token, start, end, False)

    # Function to write a set of demand matricies to text files to be used in the cloud-te formulations
    # start - datetime object
    # end - datetime object
    # interval - integer value in minutes, must be a multiple of 3
    def granularity(self, start, end):
        return
